...
class Camera:
    ...
    def process_keyboard(self, direction, velocity):
        ...
        Args:
            direction (str): Direction of movement ('FORWARD', 'BACKWARD', 'LEFT', 'RIGHT', 'UP', 'DOWN').
            velocity (float): Velocity of the movement.
        """
        if direction == 'FORWARD':
            self.position += self.forward * velocity * SPEED
        if direction == 'BACKWARD':
            self.position -= self.forward * velocity * SPEED
        if direction == 'LEFT':
            self.position -= self.right * velocity * SPEED
        if direction == 'RIGHT':
            self.position += self.right * velocity * SPEED
        if direction == 'UP':
            self.position += self.up * velocity * SPEED
        if direction == 'DOWN':
            self.position -= self.up * velocity * SPEED
